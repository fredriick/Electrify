  ...
  return (
    <DashboardLayout>
      <div className="p-6">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
              <PieChart className="w-7 h-7 text-primary-600" /> Customer Segments
            </h1>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{segmentsService.formatCurrency(stats.totalRevenue, currentCurrency)}</p>
              </div>
            </div>
          </div>
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-white">{segmentsService.formatCurrency(stats.averageOrderValue, currentCurrency)}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {segments.map((seg) => (
            <div key={seg.name} className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 flex flex-col items-center">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 ${seg.color}`}>
                <Users className="w-6 h-6 text-white" />
              </div>
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">{seg.name}</h2>
              <p className="text-gray-500 dark:text-gray-400 text-sm mb-2 text-center">{seg.description}</p>
              <div className="flex items-center gap-2 mt-2">
                <span className="text-2xl font-bold text-gray-900 dark:text-white">{seg.customers}</span>
                <span className="text-sm text-gray-500 dark:text-gray-400">customers</span>
              </div>
              <div className="mt-2 text-sm font-medium text-gray-700 dark:text-gray-300">{seg.percentage}% of total</div>
        ...
        <div className="flex flex-col items-center">
          <div className="w-40 h-40 rounded-full bg-gradient-to-r from-blue-500 via-green-500 to-purple-500 relative overflow-hidden mb-4">
          ...
          <div className="flex gap-6">
            {segments.map((seg) => (
              <div key={seg.name} className="flex items-center gap-2">
                ...
                <span className="text-sm text-gray-700 dark:text-gray-300">{seg.name}</span>
              </div>
            ))}
          </div>
        </div>

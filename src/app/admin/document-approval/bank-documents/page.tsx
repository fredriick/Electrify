...
interface PaymentMethod {
  id: string;
  supplier_id: string;
  method_type: 'bank_account' | 'mobile_money' | 'crypto';
  account_name: string;
...
export default function BankDocumentsPage() {
  const [suppliers, setSuppliers] = useState<BankDocument[]>([]);
  ...
  const [selectedSupplier, setSelectedSupplier] = useState<BankDocument | null>(null);
  ...
  const getDocumentStatus = (supplier: BankDocument) => {
    const hasBankDocument = !!supplier.bank_document;
    const supplierPaymentMethods = paymentMethods.filter(pm => pm.supplier_id === supplier.id);
    const hasPaymentMethods = supplierPaymentMethods.length > 0;
    const hasVerifiedPaymentMethods = supplierPaymentMethods.some(pm => pm.is_verified === true);
    
    if (hasBankDocument && hasVerifiedPaymentMethods) {
      return { status: 'complete', text: 'Complete', color: 'text-green-600' };
    } else if (hasBankDocument || hasPaymentMethods) {
  ...
  const getPaymentMethodTypeIcon = (type: string) => {
    switch (type) {
      case 'bank_account':
  ...
  const filteredSuppliers = suppliers.filter(supplier => {
    ...
    const hasStatus = supplier.section_approval_status?.payment === statusFilter;
    const docStatus = getDocumentStatus(supplier);
  ...
  const stats = {
    ...
    pending: suppliers.filter(s => s.section_approval_status?.payment === 'pending' || !s.section_approval_status?.payment).length,
    approved: suppliers.filter(s => s.section_approval_status?.payment === 'approved').length,
    rejected: suppliers.filter(s => s.section_approval_status?.payment === 'rejected').length,
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Bank Documents</h1>
              <p className="text-gray-600 dark:text-gray-300 mt-1">Review and approve banking documents and payment verification</p>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4">
          <div className="flex-1">
            <div className="relative">
              ...
              <input
                type="text"
                placeholder="Search by email, shop name, or company..."
                value={searchTerm}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex flex-col gap-1">
                          {supplierPaymentMethods.length > 0 ? (
                            supplierPaymentMethods.slice(0, 2).map((pm) => (
                              <div key={pm.id} className="flex items-center gap-2">
                                {getPaymentMethodTypeIcon(pm.method_type || 'unknown')}
                                <span className="text-xs">
                                  {getPaymentMethodTypeText(pm.method_type || 'unknown')}: {pm.account_number ? pm.account_number.slice(-4) : 'N/A'}
                                </span>
                                {pm.is_verified === true && <CheckCircle className="w-3 h-3 text-green-600" />}
                              </div>
                            ))
                          ) : (
                            <span className="text-xs text-gray-500">No payment methods</span>
                          )}
                          {supplierPaymentMethods.length > 2 && (
                            <span className="text-xs text-gray-500">+{supplierPaymentMethods.length - 2} more</span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs">
                              Bank Doc: {supplier.bank_document ? '✅' : '❌'}
                            </span>
                          </div>
                          ...
                          {supplierPaymentMethods.length > 0 ? (
                            <>
                              <div className="text-xs">
                                Verified: {supplierPaymentMethods.filter(pm => pm.is_verified === true).length}/{supplierPaymentMethods.length}
                              </div>
                              <div className="text-xs">
                                Primary: {supplierPaymentMethods.filter(pm => pm.is_primary === true).length > 0 ? '✅' : '❌'}
                              </div>
                            </>
                          ) : (
              ...
              <div className="mt-3">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-medium text-gray-900 dark:text-white">
                    Review Bank Documents: {selectedSupplier.shop_name || selectedSupplier.company_name}
                  </h3>
                  <button
                ...
                <div className="space-y-4">
                  ...
                  <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                    ...
                    {/* Payment Methods */}
                    <div className="mb-3">
                      <div className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Payment Methods:</div>
                      {paymentMethods.filter(pm => pm.supplier_id === selectedSupplier.id).length > 0 ? (
                        <div className="space-y-2">
                          {paymentMethods.filter(pm => pm.supplier_id === selectedSupplier.id).map((pm) => (
                            <div key={pm.id} className="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-700 rounded">
                              <div className="flex items-center gap-2">
                                {getPaymentMethodTypeIcon(pm.method_type || 'unknown')}
                                <div>
                                  <div className="text-sm font-medium">
                                    {getPaymentMethodTypeText(pm.method_type || 'unknown')}
                                  </div>
                                  <div className="text-xs text-gray-500">
                                    {pm.account_name || 'N/A'} - {pm.account_number || 'N/A'}
                                  </div>
                                  {pm.bank_name && (
                                    <div className="text-xs text-gray-500">
                                      {pm.bank_name}
                        ...
                        <div className="text-sm text-gray-500">No payment methods configured</div>
                      )}
                    </div>
                    ...
                    <div className="mb-3">
                      ...
                      {selectedSupplier.bank_document && (
                        <div className="flex items-center gap-2">
                          <FileText className="w-4 h-4 text-blue-600" />
                          <a 
                            href={selectedSupplier.bank_document} 
                            target="_blank" 
            ...
            <div className="relative top-20 mx-auto p-5 border w-11/12 md:w-1/2 lg:w-1/3 shadow-lg rounded-md bg-white dark:bg-gray-800">
              <div className="mt-3">
                <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4">
                  Confirm {selectedAction === 'approved' ? 'Approval' : selectedAction === 'rejected' ? 'Rejection' : 'Review'} for Bank Documents
                </h3>
                ...
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    placeholder={`Add notes about why these bank documents are being ${selectedAction === 'approved' ? 'approved' : selectedAction === 'rejected' ? 'rejected' : 'marked for review'}...`}
